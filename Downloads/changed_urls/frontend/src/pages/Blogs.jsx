...
export default function Blogs() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        // const response = await axios.get("https://ocd-deploy.onrender.com/blogs?page=1&limit=3");
        ...
        const withImages = await Promise.all(
          blogs.map(async (b) => {
            const details = await axios.get(`https://ocd-deploy.onrender.com/blogs/${b.id}`);
